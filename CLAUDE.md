...
- **Clicks can be tested headless:** `ClickProbeWindow` (`VGNTests/UI/FilterChipsClickTests.swift`) hosts a view in an off-screen window shaped like the app's (unified toolbar, full-size content, never key) and sends real mouse events. Use it for any "is this control actually clickable?" question — model tests cannot see hit-testing bugs. **Never let a synthetic click land on a `Menu`, a menu-style `Picker` or a pop-up button**: it opens a REAL menu on the owner's screen and blocks the test run until someone dismisses it — restrict sweeps to the region that holds the buttons under test (or click a known tooltip rect). Known trap it guards: a **horizontal `ScrollView` whose top edge touches the window toolbar never delivers clicks to its buttons** (vertical ones are fine) — use a wrapping row (`RankingFlowLayout`) there. Likewise never stack `onTapGesture` + `TapGesture().modifiers(…)`: the plain tap wins; read the modifiers inside one tap handler (`GameCell.clickKind`). **Locating a target: SwiftUI does not build its accessibility peer tree in an off-screen, never-key window, so `A11yID`/label lookup does not work headless — reach a control by a bounded coordinate sweep of a region provably free of menus (every click pop-up-guarded), or by an `NSSegmentedControl` segment frame; never sweep a whole window** (`VGNTests/UI/ClickProbeLocator.swift`). **Never fixed-sleep in a hosted-window test:** after a synthetic click, poll the post-condition across run-loop turns (`clickAndAwait` / `sweepUntil` / the `sweep*` band helpers, which stop at the first success and stay bounded), and wait for layout with `settle()`/`settleShort()`/`awaitReady` (view-tree stable across two run-loop turns) — never a fixed `Task.sleep` tier, which both misses under parallel load and burns time (wave 19).
- **Never put `fixedSize(vertical: true)` (or any unbounded ideal height) on wrapping text inside the `NavigationSplitView` DETAIL column** — the split view adopts that ideal for BOTH columns and pushes the sidebar up under the title bar (owner bug, waves 17 + 19); bound such text with `lineLimit` + a max width, keep the detail's scroll view as its top view, and note `RootView` wraps the destination area in a `GeometryReader` as a structural backstop (`.frame(maxHeight:)`/`idealHeight` do NOT contain the leak). `SidebarScrollTests` + `SidebarJumpMatrixTests` run the full selection × (loading/empty/populated) matrix and inject a deliberately-bad child.
