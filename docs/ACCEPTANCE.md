...
### "Owns multiple copies" filter (2026-09-19, wave 9 lane C)
- [ ] Toolbar **Copies ▾ ▸ "Owns multiple copies"** narrows the grid to games you own in **more than one** copy or format (≥ 2 owned products — e.g. physical + digital). It shows a removable **"Multiple Copies"** chip, fills the menu icon, ANDs with the other facets and the scope, and clears via the menu's Clear / "Clear all". A game owned once (or only played) does not appear.
