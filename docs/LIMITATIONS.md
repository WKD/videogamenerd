...
- **Subscription copies (rule 4) not yet enforced.** The merge collapses/keeps copies by platform + format + source; it does **not** protect a `products.subscription` copy from collapsing into a non-subscription one, because that column arrives from the PSN lane and does not exist yet. `ReconcileCopy` and the merge SQL carry a TODO; once `products.subscription` lands, read it in `LibraryStore.copies(of:)`, add it to `ReconcileCopy`, and make `MergePlanner`/`collapseCopy` never collapse across the subscription flag. Also add `subscription` to `ReconcileProductRow`'s round-trip (it is captured via `SELECT *` so it already survives undo — but re-check).
