...
- **UI smoke suite** (`VGNUITests`, scheme `VGN-UITests`, `scripts/uitests.sh`): **all 12 classes pass for real (2026-09-25, wave 22)** — 23 tests, 22 pass + 1 intended skip (Tier Board drag), in ONE `xcodebuild` run (≈ 3 min); per-class table in `docs/uitests.md` "Run status — 2026-09-25". The UI-test target needs `ENABLE_HARDENED_RUNTIME = NO`.
  - **Root cause of every earlier "no window" failure** (the 2026-09-25 02:23 run, and the old "only the first test of a run gets a frontmost-queryable window" blocker): XCUITest launches the app without activating it; AppKit restores the saved main window, SwiftUI's restorer returns nil and SwiftUI then opens **no** default window — the app ran with a menu bar and no window. The tests now launch with `-ApplePersistenceIgnoreState YES`. The same hit a real **background launch** (`open -g`, a login item): menu bar, no window until the Dock icon was clicked. **[done, wave 23]** — `MainWindowGuard` (`VGN/UI/Support/MainWindowGuard.swift`, an `NSApplicationDelegateAdaptor` + a marker view in each main window): ~0.4 s after launch and on the first activation, if no main window is visible or minimised (checked twice, a beat apart, so a restored window is never doubled) it opens the main `WindowGroup` (now `id: "library"`) via `openWindow`. State restoration stays on, so the window frame is still remembered. `LaunchSmokeTests.testLaunchWithSavedStateStillShowsWindow` launches twice without the flag (not yet run — see `docs/uitests.md`). Note: changing the `WindowGroup` id means the first launch after this update does not restore the old window's frame (the guard opens a default-sized one).
