...
| UI smoke suite | Run it **only when the owner says they are away from the Mac** (≈ 3 min of keyboard/mouse takeover for the whole suite). **Green since wave 22 (2026-09-25)**: `scripts/uitests.sh` = 23 tests, 22 pass + 1 intended skip, in one run; `--per-class` still available for isolation. **[done]** |
...
| Disc badge on cartridges *(2026-09-25)* | **Accepted as is** — the Physical badge is a disc on cartridge games too; not a defect, no cartridge variant planned. |
| PSN *(2026-09-25)* | Real-library import done, `m7` tagged. The `psn-test` profile is retired (left on disk, unused). |
...
- **UI smoke suite** (`VGNUITests`, scheme `VGN-UITests`, `scripts/uitests.sh`): **all 12 classes pass for real (2026-09-25, wave 22)** — 23 tests, 22 pass + 1 intended skip (Tier Board drag), in ONE `xcodebuild` run (≈ 3 min); per-class table in `docs/uitests.md` "Run status — 2026-09-25". The UI-test target needs `ENABLE_HARDENED_RUNTIME = NO`.
  - **Root cause of every earlier "no window" failure** (the 2026-09-25 02:23 run, and the old "only the first test of a run gets a frontmost-queryable window" blocker): XCUITest launches the app without activating it; AppKit restores the saved main window, SwiftUI's restorer returns nil and SwiftUI then opens **no** default window — the app ran with a menu bar and no window. The tests now launch with `-ApplePersistenceIgnoreState YES`. The same hit a real **background launch** (`open -g`, a login item): menu bar, no window until the Dock icon was clicked. **[done, wave 23 — W23-B]** Real root cause (from the app's `StateRestoration` log): an id-less `WindowGroup`'s saved identifier is the **type name of the root view's modifier chain** (`SwiftUI._ConditionalContent<…RootView…>-1-AppWindow-1`), so any build that changes those modifiers makes the saved state unmappable → restorer `window=0x0` → no default window. Fix: (1) the main `WindowGroup` has a stable id `"library"` (saved as `library-AppWindow-1`, survives rebuilds); (2) `MainWindowGuard` (`VGN/UI/Support/MainWindowGuard.swift`, `NSApplicationDelegateAdaptor` + a marker view per main window) — ~0.4 s after launch and on first activation, if no main window is visible/minimised (checked twice, a beat apart, so a restored window is never doubled) it calls SwiftUI's own delegate `applicationOpenUntitledFile(_:)`. The first W23-A version captured `openWindow` from a `Commands` struct instead; with no window SwiftUI never evaluates the commands, so it opened nothing (the 2026-09-26 00:10 smoke failure). Verified by hand in sample mode: `open -g` with stale state (simulated by a build with another scene id → restorer `window=0x0`) → exactly one window, guard log "opened one: true"; `open -g` after a normal quit → restored window, guard does nothing (one window). State restoration stays on; the first launch after this update opens at the default size once.
  - **App fixes it found:** ⌘F / View ▸ Find did not focus the toolbar search field (typing went to the sidebar); tiered grid cells exposed **no** accessibility value (the tier chip's AppKit tooltip view broke the cell's combined element — VoiceOver lost "Tier A, Played" too); container identifiers overrode their children's (scan sheet, filter-chips bar). **[done]**
  - ~~**Still open (app):** Play Next shows "Nothing to play here yet" when every candidate merely lacks a length estimate; Triage snapshots its queue on appear.~~ **[done, wave 23]** Play Next: `PlayNextResult.isEmpty` now counts the unknown-length lane; `PlayNextModel.contentState` (`noRankings / empty / onlyUnknownLength / picks`) drives the page; the only-unknown state reads "None of your unfinished games has a length yet", offers **Fetch Missing Time Estimates…** (`HLTBFetchPresenter.presentBulkForAllMissing`, every game without an estimate — the grid's own bulk entry still scopes to its selection) and lists the lane expanded; each unknown-length card leads with "No time estimate — fetch from HowLongToBeat". Triage: `TriageModel.observeLibrary()` follows the unranked-games DB observation for as long as the view's `.task` lives (no timers); games tiered/un-played elsewhere leave, newly eligible ones are appended, the current card is never swapped (if it becomes ineligible Triage moves on with a one-line note); emissions during Triage's own writes are held and replaced by one fresh read.
  - **Snapshot drift (wave 23):** `settings-accounts@light/dark` (10–14 %) was the wave-22 **"Your pace"** section (`PaceFactorRow`) shifting the captured part of the Settings pane — correct content, **re-recorded** (only those two references); full `scripts/snapshots.sh` verify then 100/100 green.
  - **Privacy (wave 22):** Xcode 26 records a **full-screen video** of each UI test and keeps full-screen failure screenshots by default — the 02:23 run's `.xcresult`s (in `main/.build/uitests/20260925-022309/`) contain such recordings of the owner's desktop and should be deleted. The scheme now sets `systemAttachmentLifetime = keepNever` + `preferredScreenCaptureFormat = screenshots`, the runner passes `-collect-test-diagnostics never`, and failure dumps are window-scoped text (never the menu bar's Recent Items). XCTest still keeps its own text triage dumps inside the local `.xcresult`; the export drops them from the gallery.
  - Do not run the UI suite while another lane runs unit tests: `XCUIApplication.launch()` may terminate a running `com.pomatelier.VideoGameNerd` (same bundle id — e.g. another worktree's unit-test host), and both drive windows. Check `pgrep -x VGN` before each class (wave 22 saw another lane's test host appear mid-session and waited for it).
