...
- **What counts as a game — `game_type` policy (wave 19, W19-E).** One pure classifier (`GameTypePolicy`) drives one function (`IGDBClient.bundleMembers(ofBundleID:)` → `BundleMemberResult`) through which every bundle-member producer flows: non-standalone content (DLC/expansion/season/pack/mod/update) is dropped and reported ("Left out: …"), standalone expansions and episodes are kept, a **port** folds onto its parent game (one paced `games(ids:)` lookup, cached), and < 2 members ⇒ not a compilation.
  ...
  - **Deferred [later]:** the *interactive* "choosing a port result links to its parent (with an insist-on-port option)" in Quick Add and the Link-to-IGDB sheet, and the *import matcher's* automatic port→parent swap (a Switch/PS4 port of an older game landing on the one game as a new copy), are **not built** — those result rows are labelled ("Port"), and a port that appears *inside a bundle* already folds correctly, but a standalone port match still links to the port entry. Needs a parent-resolution seam on `CatalogSearching`/`ImportMatcher` (and `ScanMatch` carrying the parent id); left for a follow-up to avoid rippling that seam through every fake this wave.
...
- ~~Deleting one copy of a multi-copy game (Mac + PC) left the removed platform's pill under the title for ever.~~ **Fixed (wave 19, lane C):** removing/re-pointing a copy now runs `LibraryStore.pruneCopyOnlyPlatforms` in the same transaction — it drops a `game_platforms` row that a copy alone put there (`played = 0`, no remaining copy on that platform), so the pill, the inspector platform list and the sidebar per-platform count all update. A launch-once repair (`repair.copyOnlyPlatforms.v1`) cleans pre-fix leftovers. **Compromise [by design]:** a platform row the owner deliberately picked at Quick Add is stored identically (`played = 0`) to one a copy added — the two are indistinguishable — so rules keep a row whenever it is marked *played on* (`played = 1`) or is the game's **last** platform (a game never ends with zero platforms). Net effect: a hand-picked, not-played, non-last platform that never had a copy could in principle be pruned by a *later* copy removal on the same game; in practice such rows only arise from copies, so this is a theoretical edge, not an observed one. [watch]
