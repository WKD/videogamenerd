...
- **Deferred (next PSN lane).**
  - **DEBUG "PSN build steps" panel (§13.5)** — the one-button-per-live-step panel (`PSNBuildStepsModel`, `test`/`real` label picker, probe-before-full lock, reject→Acknowledge lock, per-step status/bytes/cache path, running total vs the 40 budget, "Wipe dev cache") is **not built yet**. The persisted account label (`PSNAccountModel.accountLabel`, key `psn.buildSteps.accountLabel`, default `test`) and the DEBUG dev-cache wiring in `PSNImportBuilder` are in place for it. Until then, the live rollout uses the ordinary **Sync Now** / **Force Refresh** flow, which still probes before every full fetch (the importer calls `client.probe(...)` first, so a full fetch is never attempted un-probed) — but it is *not* one-button-at-a-time, so the orchestrator must run S1–S8 deliberately (sign in, then Sync Now once per account, watching the reject surface).
  - **Richer PSN review grouping (§13.3)** — the review sheet shows the generic *New / Already matched / Ignored* buckets. The PSN-specific groups (**Played** pre-ticked, **Launched 0 %** unticked, **Played — no purchase found** with the "Own the ticked rows as ▸ Physical/Digital" group action, **Purchased**, **PS Plus**, **Already in your library** with per-row change detail, **proposed removals** of disappeared PS Plus claims as their own confirm group) are **not built**. The commit is already correct per row; `ImportStagingStore.proposedSubscriptionRemovals` exists (data side) but is not surfaced. A ticked *launched-0 %* row currently commits as a no-op (safe).
  - **Play Next PS Plus term (§13.3)** — the "leaves with PS Plus" reason line and the persisted **Options ▸ Prefer expiring PS Plus games** gated scoring term are **not built**. `GameSummary.ownedOnlyViaSubscription` / `GameDetail.ownedOnlyViaSubscription` are available for the reason line; the term must be additive, gated by the option, and excluded from the backtest.
