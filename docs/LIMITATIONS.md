...
- **Usage**: ≈ 8 `claude -p` calls and ~1–2 minutes per photo; the CLI priced the 5-photo run at $19.66 *notional* (≈ $4/photo on the default model) — it counts against your subscription's usage limits, it is not charged (see the note in §9). A cheaper model has **not** been evaluated. [decide: test one photo with a cheaper model]
