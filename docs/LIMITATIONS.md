...
| Grid: letters vs type-to-select | **Tier / owned / played keys need ⇧** in the library grid (`⇧S…⇧F`, `⇧O`, `⇧P`); plain letters always type-to-select. `0` still clears the tier. Tier Board, Triage and Quick Add keep their own keys. **[doing]** |
...
| Photo-scan tile size | **Measure smaller tiles on one photo** (IMG_3683, ≤ 1 500 px, ~15 calls) before deciding; current tiles are 1850 × 2400 px and Claude views images at roughly 1 568 px on the long edge (~65 % scale). **[doing]** |
