...
- **Still to wire (see `docs/LIMITATIONS.md §5b`).** The background IGDB trait pass for PS Plus entries, the browser's PS-Plus row (remote cover / marker / actions / PS Store link), the Play Next "From the vault" row over both sources, the Settings deadline picker, the review-sheet "In the Vault (N)" collapsed group, and promotion-on-play linking.
