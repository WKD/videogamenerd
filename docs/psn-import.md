...
- **2026-09-25 — REAL LIBRARY import done (owner): "PSN import worked perfectly".** The first import into the owner's real library went through the review sheet without a reject or stop; the library grew to 988 games (psn-sourced play time on 179 games). §13.6 "done when" is met → tag **`m7`**. The `psn-test` profile (`-VGNProfile psn-test`) is **retired** — the owner will not use it again; its data folder is left untouched (never deleted by us).
