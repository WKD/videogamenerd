...
enum PSNMapping {
    ...
    /// Drop a trailing platform tail Sony appends for MATCHING only (PLAN §13.3 / D5) — the pure
    /// logic now lives in ``PlatformTail`` (`VGN/Matching/`) so the reconcile link-sheet prefill
    /// shares it. Kept as a forwarder so existing PSN callers are unchanged.
    static func dropPlatformTail(_ title: String) -> String {
        PlatformTail.drop(title)
    }
