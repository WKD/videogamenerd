...
**Open questions.** Which Recalbox version (is `timeplayed` there, or only `playcount` / `lastplayed`)? How do I reach the files (SMB mount vs a copied folder) and should a re-sync be manual like every other importer (yes)? Arcade sets (MAME/FBNeo romsets are not game lists — probably systems to skip or treat separately)? How good is title intersection for Japanese-only and translated ROMs? Does the scraper data already in `gamelist.xml` (rating, genre, description, image) make IGDB unnecessary for the browse view (likely yes — IGDB only for the Discover pool)?

**First step when scheduled:** inspect a real `gamelist.xml` sample (like the Delicious file was inspected), then a reader + dry-run report — counts per system, played vs never played, how many collapse as duplicates, intersection size per platform — before any UI.
