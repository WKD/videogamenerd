...
- LIBRARY also carries a **Bundles to Expand** row *(2026-09-20, wave 16)* — right under Unlinked, games whose title looks like an unexpanded bundle (§5.1), shown **only when its count > 0**, counted by the same single aggregate observation. Selecting it scopes the grid to the candidate ids (a Swift title heuristic, so the ids are computed once per library change and passed to the query) with a slim explanatory header; expand a game from the grid context menu, inspector or File menu.
...
**The PSN "Launched" group: unticked means Vault** *(owner, 2026-09-20 — wave 17)*. One exception to "unticked = not this time": in the PSN review's **Launched** group (trophy list at 0 % and/or play time ≤ 10 min — the same `ImportPolicy.vaultPlaytimeGateSeconds` gate) the rows are unticked by default, and leaving them unticked is already my decision that they are not backlog. So at commit, every unticked, not-ignored row of that group is **sent to the Vault** automatically (same persisted decision and undo as the explicit action; `owned = 1` unless the row is a PS Plus claim, which stays a subscription claim and keeps its deadline boost). The group header says so ("Unticked rows go to the Vault") with a switch to turn it off for this commit; the result banner reads "N imported · M sent to the Vault". Example that prompted it: *Prey* — a disc game (game-list `service = other`), launched 8 min, trophies 0 %. A launched game I no longer have access to (lent, sold) is removed from the Vault with **Not Interested**, or kept out with **Ignore** in the review. Every other group keeps the three fates above.

**Format `rom` stays** *(owner decision, 2026-09-20: "keep ROM format, everything from Batocera is a ROM")*. Every copy created from Batocera (review promotion, favourites auto-add, Vault ▸ Add to Library…) is `format = rom`, never digital. A ROM copy is not a purchase: it is what separates "files on my emulation box" from "licences I paid for" in Format ▸ Digital, the Stats ownership split, the multiple-copies rule (a ROM + a cartridge of the same game are two different things worth seeing) and Batocera promotion (`gameHasROMCopy`, the purple badge). Folding it into Digital is a one-way migration — hand-entered ROM copies (`source = manual`) could never be told apart again. If it is only the extra menu entry that bothers, hide Format ▸ ROM when the library has no ROM copy.
