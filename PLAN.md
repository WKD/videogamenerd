...
- **Counts as owned, but says so**: the game is in Owned/Backlog like any digital copy; the grid cell shows a small **"+" badge** (PS Plus yellow) next to the owned badge, the inspector's copy row reads "PS Plus — expires with the subscription", and export carries the flag. A game I own **only** through PS Plus is what matters: a copy I also own on disc is not at risk and shows no badge.
