...
4. Manual: "Choose cover…" sheet showing every candidate from every provider, or drag any image onto the game. *(wave 17)* IGDB contributes both the game's cover **and** its `artworks` (fetched on demand when the sheet opens, live only), grouped as provider · kind · size; landscape artworks preview with the grid's portrait crop.

*(wave 17)* An importer-supplied cover (Delicious box art) is **provisional**: the background cover job still runs and upgrades it once the provider chain finds a real cover, but a hand-picked cover is never replaced and a miss keeps the importer cover (`games.cover_provisional`, migration v14).
